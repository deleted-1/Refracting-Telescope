...
def set_parameters():
    ...
    def show_diagram():
        try:
            if int(distance_object.get()) <= int(focal_objective.get())*2:    Label(root, text="Has to be twice focal objective!", font=("time new roman",10, "italic"), fg='red').place(x=20,y=45)
            elif float((1/float(focal_objective.get())- 1/float(distance_object.get()))**-1)  < float(focal_ocular.get()):   Label(root, text="Please specify other inputs as the real image is beyond the focal of the ocular!", font=("time new roman",10, "italic"), fg='red').place(x=20,y=145)
            elif int(distance_object.get()) > 200:  Label(root,text="Please choose an integer smaller or equal to 200 for object distance.", font=("time new roman",10, "italic"), fg='red').place(x=20,y=45)
            elif int(len_distance.get())-int(focal_ocular.get()) <= int(focal_objective.get()) or int(focal_ocular.get()) <= 0: Label(root, text="Please enter a larger focal ocular.", font=("time new roman",10, "italic"), fg='red').place(x=20,y=95) 
